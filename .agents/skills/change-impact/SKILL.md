...
- 跨模块、公共接口、数据迁移、认证、部署或不可逆变更开始前。
- 用户询问 blast radius、迁移风险、回滚方式或“会影响哪里”。
- 实施完成后需要把计划影响与实际 diff 对照。

## Anti-Patterns

- 仅凭目录名或模型常识猜调用关系。
- 把影响报告写成另一份会漂移的需求或架构文档。
- 在没有恢复步骤、兼容窗口和不可逆说明时宣称高风险变更可交付。

## Related Skills

- `architecture-decision-records`：需要长期保留理由的难回退决策。
- `contract-first`：跨服务或跨端边界变化。
- `test-collaboration`：成功标准、风险和缺陷的验证证据。
- `module-regression`：实施后的下游回归。
...
- 架构、数据库、认证、部署拓扑或长期技术策略：检查 `architecture-decision-records` / ADR。
