...
This directory documents ECC (Everything Claude Code) support for the current Kimi Code CLI. The managed adapter is verified against Kimi Code 0.31.x (`@moonshot-ai/kimi-code`).
...
- Kimi Code 0.31.x does not expose a supported ECC lifecycle-hook install surface, so this adapter does not offer or claim hook profiles.
