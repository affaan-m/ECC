...
function runTests() {
  ...
  if (test('hooks.json exposes one sync and one async PostToolUse entry', () => {
  ...
  if (test('dry-run selects the original IDs by tool and phase', () => {
    const cases = [
      {
        tool: 'Edit',
        sync: [
  ...
  if (test('actual hooks.json commands preserve Edit dry-run output and IDs', () => {
    const entries = JSON.parse(fs.readFileSync(hooksPath, 'utf8')).hooks.PostToolUse;
    const raw = JSON.stringify({
      hook_event_name: 'PostToolUse',
      tool_name: 'Edit',
  ...
  if (test('actual hooks.json commands never echo truncated oversized input', () => {
    const entries = JSON.parse(fs.readFileSync(hooksPath, 'utf8')).hooks.PostToolUse;
    const values = [
  ...
  if (test('profiles and disabled IDs remain scoped to each original hook', () => {
  ...
  if (test('public dispatcher IDs disable their complete phase', () => {
    const entries = JSON.parse(fs.readFileSync(hooksPath, 'utf8')).hooks.PostToolUse;
    const raw = JSON.stringify({
      hook_event_name: 'PostToolUse',
      tool_name: 'Edit',
  ...
  if (test('dry-run has no PostToolUse side effects', () => {
  ...
  if (test('dispatcher isolates failures and preserves explicit output and exit status', () => {
    ...
    const hooks = [
      { id: 'post:test:first', matcher: '*', profiles: 'standard,strict', run: input => { calls.push(['first', input]); return input; } },
      { id: 'post:test:broken', matcher: '*', profiles: 'standard,strict', run: () => { throw new Error('boom'); } },
      { id: 'post:test:nonzero', matcher: '*', profiles: 'standard,strict', run: () => ({ exitCode: 7 }) },
      { id: 'post:test:last', matcher: '*', profiles: 'standard,strict', run: input => { calls.push(['last', input]); return { stdout: explicitOutput, stderr: 'last warning' }; } },
    ];

    const result = runHooks(raw, hooks, { toolName: 'Read', env: { ECC_HOOK_PROFILE: 'standard' } });
    assert.deepStrictEqual(calls, [['first', raw], ['last', raw]], 'each hook should receive the original input');
    ...
    assert.strictEqual(
      resolveMainStdout(raw, { stdout: '', exitCode: 7 }, { passthrough: true, truncated: false }),
      '',
      'nonzero results should not restore raw input'
    );
  })) passed++; else failed++;

  if (test('console warning hook is safe to require in-process', () => {
    const script = [
      `const hook = require(${JSON.stringify(path.join(repoRoot, 'scripts', 'hooks', 'post-edit-console-warn.js'))});`,
      "if (typeof hook.run !== 'function') process.exit(2);",
    ].join('');
  ...
  if (test('empty and malformed input fail open', () => {
