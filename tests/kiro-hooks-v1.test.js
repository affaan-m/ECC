...
function runMatcherTests() {
  ...
  // security-check-on-create: (^|/)(auth|api|middleware)(/|$)
  const securityMatcher = '(^|/)(auth|api|middleware)(/|$)';
  ...
  // typecheck-on-edit: \.(ts|tsx)$
  const typecheckMatcher = '\\.(ts|tsx)$';
  ...
  // doc-file-warning: (README|CHANGELOG|docs/|\.md$)
  const docMatcher = '(README|CHANGELOG|docs/|\\.md$)';
  ...
  // git-push-review: execute_bash (tool name matcher)
  const gitPushMatcher = 'execute_bash';
  ...
  // code-review-on-write: fs_write|str_replace|fs_append (tool name matcher)
  const codeReviewMatcher = 'fs_write|str_replace|fs_append';
