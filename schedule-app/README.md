...
| **Map** | Drop pins with an emoji + text label anywhere on the map. Tap a pin for its info card and a **Get directions** button (opens Google Maps navigation), plus a corner directions button. Pins can be linked to a person, and a "my location" button re-centers the map. |
