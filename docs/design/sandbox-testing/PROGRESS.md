...
| final | executable automated review | Greptile reproduced a managed Lume launcher exit that left its guest running before deletion | Fixed; cleanup now stops the launcher tree, explicitly stops the guest, then deletes it, with a dedicated ordering regression |
