...
| Codex | Adapter-backed | `AGENTS.md`; Codex plugin metadata; skills; MCP reference config; command patterns; native session context hooks; native continuous-learning lifecycle hooks | Blocking Claude hook enforcement is not projected; Claude slash-command semantics are not equivalent | `./install.sh --profile minimal --target codex`; repo-local `AGENTS.md` review | `node tests/codex-native-hooks.test.js`; `npm run harness:audit -- --format json` | Codex requires explicit trust for native hook commands; Only the context and continuous-learning lifecycle is projected. |
