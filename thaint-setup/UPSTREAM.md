...
| Upstream `main` last checked | `591ab5cb` = `v2.1.0-15` (2026-07-28) — merges clean |
...
`v2.1.0..591ab5cb` — 15 commits, 33 files, +4629 / −1456. No newer tag yet.
...
Docs: README restructured for 2.1 (#2579), badges for non-existent
`api.ecc.tools` endpoints dropped, TDD section retitled.

Deps: cargo minor/patch group (#2593), pyyaml ≥6.0.3 (#2455), pytest-mock
≥3.15.1 (#2454).

Overlaps our changes in 4 files — `scripts/lib/utils.js`,
`tests/hooks/cost-tracker.test.js`, `tests/hooks/hooks.test.js`,
`tests/lib/utils.test.js`. Merge is clean today, but this is where a conflict
would land. The session-id logic in `utils.js` is untouched, so that fix is still
needed.
