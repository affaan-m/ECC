...
Resolve the active ECC root with the same chain used by hook bootstrap and
other slash commands, then run the instinct CLI. This avoids reading a stale
manual install when `CLAUDE_PLUGIN_ROOT` is unavailable but the plugin is active
from `~/.claude/plugins/cache/...` (#2037).
