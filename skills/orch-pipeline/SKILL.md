...
The `orch-*` skills are thin wrappers. They do not re-implement any work â€” they
...
> Invoke an operation skill (`orch-add-feature`, `orch-fix-defect`, â€¦) rather
...
| `orch-build-mvp` | mvp | bootstrap from a design/spec doc | ingest doc â†’ vertical slices |
...
## Step 0 â€” Classify size (right-sizing)
...
|------|---------------|---------------------------|------------------|-----------------|
| trivial | 1, a few lines | none | none â€” the change is obvious | 4 â†’ 5 â†’ 6 |
| small | 1 file / 1 function | none | clear once you read the code | (1 light) â†’ 4 â†’ 5 â†’ 6 |
| standard | 2â€“5 files | maybe a new internal module | one real choice to make | 1 â†’ 2 â†’ 4 â†’ 5 â†’ 6 |
| large | many / cross-cutting | new external dep, public API, or a spec doc | multiple open questions | 1 â†’ 2 â†’ (3) â†’ 4 â†’ 5 â†’ 6 |
...
Each phase delegates â€” it does not do the work inline.

- **0. Intake** â€” restate the request. For `orch-build-mvp`, read the spec/design
  doc and extract scope, locked decisions, and a feature list.
- **1. Research & Reuse** â€” per `rules/common/development-workflow.md`: `gh search repos` /
...
- **2. Plan** â€” delegate to the `planner` agent (or `architect` /
  `code-architect` for structural decisions). Output a `task_list` ordered as
  thin vertical slices. â†’ **GATE 1.**
- **3. Scaffold** â€” `orch-build-mvp` only: stand up the first end-to-end slice.
- **4. Implement (TDD)** â€” drive each task through the `tdd-guide` agent (or the `tdd-workflow` skill):
  red â†’ green â†’ refactor. Honor the operation's first-move rule.
- **5. Review** â€” `code-reviewer` agent / `/code-review`. Add `security-reviewer`
  whenever the diff touches a security trigger (below).
- **6. Commit** â€” conventional commits (`feat:` / `fix:` / `refactor:` / â€¦), one
  per logical chunk. â†’ **GATE 2.**

## The two gates

This family is **gated, not autonomous**:

1. **GATE 1 â€” after Plan.** Present the `task_list`; do not write implementation
   code until the user approves.
2. **GATE 2 â€” before Commit.** Present the diff summary and proposed messages;
...
| Review | `code-reviewer` / `/code-review` | language reviewer (`python-reviewer`, `typescript-reviewer`, â€¦) |
| Security | `security-reviewer` | â€” |
| MVP inner loop | `/gan-build "<brief>" --skip-planner` | drives `gan-generator` â†’ `gan-evaluator`; tune `--max-iterations` / `--pass-threshold` |
...
The pipeline carries no hidden state â€” the planning docs *are* the handoff:
...
- new / changed behavior has tests; coverage â‰¥ 80% per `rules/common/testing.md`
