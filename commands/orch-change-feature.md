...
2. Light plan only if the new behavior needs research. â†’ **GATE 1** (approve changed-test plan).
3. **Update the existing tests** to express the new behavior, then change the
   ...
   tweak, not a fix.)
4. `code-reviewer` (+ `security-reviewer` on a security trigger), then commit. â†’ **GATE 2**.

Use this only when the feature **works** but should behave differently â€” not for
