...
const fs = require('fs');
const { test, createTestDir, cleanupTestDir, withTestDir, runValidatorWithDir, runValidatorWithDirs, runValidator, finish } = require('./validator-test-utils');

// Create the three fixture dirs the commands validator needs and always
// clean them up, even when an assertion throws mid-test.
function withCommandDirs(fn) {
  return withTestDir(testDir => withTestDir(agentsDir => withTestDir(skillsDir => fn(testDir, agentsDir, skillsDir))));
}
...
test('detects broken command cross-reference', () =>
  withCommandDirs((testDir, agentsDir, skillsDir) => {
...
console.log('\nvalidate-commands.js (multi-reference lines):');
...
console.log('\nvalidate-commands.js (no references):');
...
console.log('\nvalidate-commands.js (workflow chains):');
...
console.log('\nvalidate-commands.js (missing agents dir):');
...
console.log('\nvalidate-commands.js (inline backtick refs):');
...
console.log('\nvalidate-commands.js (workflow whitespace):');
...
console.log('\nvalidate-commands.js (adjacent code blocks both stripped):');
...
console.log('\nvalidate-commands.js (unreadable command file):');
...
console.log('\nvalidate-commands.js (empty commands directory):');
...
console.log('\nvalidate-commands.js (would create: skip):');
...
test('skips command references on "would create:" lines', () =>
  withCommandDirs((testDir, agentsDir, skillsDir) => {
    ...
    // Only "creates:" was previously tested. "Would create:" exercises
...
console.log('\nvalidate-commands.js (unreadable skill entry — statSync catch):');
...
console.log('\nvalidate-commands.js (warnings count in output):');
