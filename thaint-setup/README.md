...
10. **Applies local patches** — see [Patches](#patches) below
...
Two patch scripts run near the end of setup. Both rewrite the **installed**
copies under `~/.claude` only, never this repo's tracked files, so the tree
stays clean for `git merge upstream/main`.

| Script | Purpose | Status at v2.1.0 |
|---|---|---|
| `patch-ecc-session-id.sh` | ECC reads `CLAUDE_SESSION_ID`; Claude Code exports `CLAUDE_CODE_SESSION_ID`. Rewrites the reads, keeping the old name as a fallback. | **Works.** 19 occurrences across 14 files. |
| `patch-ecc-suggest-compact.sh` | Re-adds the `[Delegation]` nudge to `suggest-compact.js`. | **Anchor no longer matches** — v2.1.0 rewrote the hook to the `messages.push(...)` / `additionalContext` shape. Warns and skips; non-fatal. Needs re-authoring. |
