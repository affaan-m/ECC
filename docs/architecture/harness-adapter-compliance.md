...
| Kiro | Adapter-backed | Kiro steering files (rules); shared skills; agents (IDE markdown + CLI JSON); native `.kiro.hook` IDE hooks; MCP server example | No native slash-command surface; ECC commands surface as skills/steering; IDE hooks use Kiro askAgent/runCommand, not Claude plugin-bootstrap hooks | `./install.sh --profile minimal --target kiro` | `node tests/lib/install-targets.test.js`; `node tests/lib/kiro-generate.test.js`; `node scripts/generate-kiro-adapter.js --check` | Regenerate `.kiro/` via `scripts/generate-kiro-adapter.js` after editing canonical sources to avoid drift. |
